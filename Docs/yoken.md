...
ネイルショップ向けのウェブサイトを構築し、LINE Login を利用した予約システムを実現する。予約は LINE アプリからのみ受け付ける。
...
1. LINE アプリから Web アプリへ遷移（LINE Login で認証）
2. 日時・メニュー選択
   - 施術内容の概要、施術時間、価格を表示
   - メニューを選択
...
- 押下時に以下を実行：
  - LINE ユーザー ID 単位の予約キャンセル回数・予約変更回数を 0 にリセット
